...
def test_search_words(mock_get_text):
    ...
    result = search_words(data, "ír", "Contains", "Partial match")
    assert len(result) == 1
    assert result.iloc[0]["Word"] == "Míreadóir"

    # Test 'begins_with' with partial match
    result = search_words(data, "F", "Begins with", "Partial match")
    assert len(result) == 1
    assert result.iloc[0]["Word"] == "Fón"

    # Test 'ends_with' with partial match
    result = search_words(data, "ch", "Ends with", "Partial match")
    assert len(result) == 1
    assert result.iloc[0]["Word"] == "Árthach"

    # Test no matches
    result = search_words(data, "xyz", "Contains", "Partial match")
    assert len(result) == 0


# Test df_to_clickable_html
